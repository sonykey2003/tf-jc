...
output "public_ip_info" {
  value = [for i in range(length(aws_instance.server-farm)) : "Server Name: ${aws_instance.server-farm[i].tags["Name"]}, Public IP: ${aws_instance.server-farm[i].public_ip}"]
}

output "public_dns_info" {
  value = [for i in range(length(aws_instance.server-farm)) : "Server Name: ${aws_instance.server-farm[i].tags["Name"]}, Public DNS: ${aws_instance.server-farm[i].public_dns}"]
}

output "private_ip_info" {
  value = [for i in range(length(aws_instance.server-farm)) : "Server Name: ${aws_instance.server-farm[i].tags["Name"]}, Private IP: ${aws_instance.server-farm[i].private_ip}"]
}
